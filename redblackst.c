...
Node *new_node(const void *key, const void *val,  Bool color, int size) {
...
Node *aux_put(Node *h, const void *key, const void *val) { 
    
    if (h == NULL) return new_node(key, val, RED, 1);


    int cmp = strCmp(key, h->key);
    if      (cmp < 0) h->left  = aux_put(h->left,  key, val); 
    else if (cmp > 0) h->right = aux_put(h->right, key, val); 
    else              h->val   = (void*)val;
...
void  put(RedBlackST st, const void *key, size_t sizeKey, const void *val, size_t sizeVal){
    ...
    st->root = aux_put(st->root, key, val);
    st->root->color = BLACK;
    // assert check();
}
...
//Node delete_node(Node h, Key key) { 
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < n; i++){
        pt = &i;
        for(int j = 0; j < 10; j++) scanf("%c",&c[j]);
        put(st, c, strlen(c) + 1, pt, sizeof(Integer)); 
    }
    return EXIT_SUCCESS;
}
*/
